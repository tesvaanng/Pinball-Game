...
| 範疇 | Manager | 職責 |
|---|---|---|
| Core | `GameCoreManager` | 共用工具：BigNumber、DeterministicRng、Singleton、初始化介面與註冊表 |
...
| Board | `BoardManager` | 盤面、多顆球、釘子、袋口、球上分數、物理事件 |
| Input | `InputManager` | 力度條輸入與發射事件 |
...
- 進度以完成數量計算，例如 4 個 initializer：完成 1 個為 0.25、2 個為 0.5，完成全部為 1。
